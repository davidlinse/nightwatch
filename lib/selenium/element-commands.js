...
var _getLocateStrategy = require('./find-element-strategy');
  ...
  function addElementCommand(protocolAction, extraArgs) {
    ...
    return function() {
      ...
      function CommandAction() {
        ...
        var strategy = _getLocateStrategy([value]);

        var el = Protocol.element(strategy.using, strategy.value, function(result) {
          if (result.status !== 0) {
            callback.call(client.api, result);
            var errorMessage = 'Unable to locate element: "' + strategy.value + '" using: ' + strategy.using;
