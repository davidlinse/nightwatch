//
// shameless copy but taken in courtesy from 'webdriver.io'
// https://github.com/davidlinse/webdriverjs/blob/master/lib/helpers/find-element-strategy.js
//
module.exports = function findStrategy(args) {
  ...
  // check value type
  // use id strategy if value starts with # and doesnt contain
  // any other CSS selector-relevant character
  if(value.indexOf('#') === 0 && value.search(/(\s|>|\.|[|])/) === -1) {

    // console.log('  --> using id-strategy for value: '+ JSON.stringify(value));
    using = 'id';
    value = value.slice(1);
    ...
    // if nothing fits with the supported strategies we fall back
    // to the css selector strategy
  } else {
    using = 'css selector';
  }
