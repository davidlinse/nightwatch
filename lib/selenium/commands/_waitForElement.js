...
WaitForElement.prototype.command = function(selector, milliseconds, callbackOrAbort) {
  ...
  this.locateStrategy = this._getLocateStrategy(selector);
...
/**
 * Auto detect `locateStrategy` based on given selector.
 * The methods the appropriate value for the #locateStrategy property.
 *
 * NOTE: There is currently no support for 'extended' selectors
 *       like: 'div.foo-class' or 'span#baz-id'.
 *
 * @param   {string}  selector
 * @returns {string} 'css selector' for 'css' selectors, 'xpath' otherwise
 */
WaitForElement.prototype._getLocateStrategy = function _getLocateStrategy(selector) {
  var regx = /^(\.|#)\w+/;  // match classical css like #body or .foo
  return regx.test(selector) ? 'css selector' : 'xpath';
};
